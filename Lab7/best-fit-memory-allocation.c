...
void bestFit(MemoryBlock blocks[], int blockCount, Process processes[], int processCount) {
    ...
    for (int i = 0; i < processCount; i++) {
        ...
        int minWaste = 99999;  // Large initial value
        
        // Find block that leaves the smallest leftover space
        for (int j = 0; j < blockCount; j++) {
            if (workingBlocks[j].allocated == -1 && workingBlocks[j].size >= processes[i].size) {
                int waste = workingBlocks[j].size - processes[i].size;
                if (waste < minWaste) {
                    minWaste = waste;
                    bestBlockIndex = j;
                }
        ...
        if (bestBlockIndex != -1) {
            workingBlocks[bestBlockIndex].allocated = processes[i].id;
            printf("Process P%d (size %d) allocated to Block B%d (size %d) with %d waste\n", 
                   processes[i].id, processes[i].size, 
                   workingBlocks[bestBlockIndex].id, workingBlocks[bestBlockIndex].size, 
                   minWaste);
        } else {
