...
typedef struct {
    int pid, at, bt, rt, ct, tat, wt, rt_time, q;
} Process;
...
void mlfq(Process p[], int n, int qs[], int m) {
    ...
    for (int i = 0; i < n; i++) p[i].rt = p[i].bt, p[i].q = 0;
    ...
    while (done < n) {
        // New processes always go to queue 0
        while (idx < n && p[idx].at <= time) 
            q_push(&queues[0], idx), in_queue[idx] = 1, p[idx].q = 0, idx++;
        ...
        int exec = (cur == m-1) ? p[i].rt : 
                   (p[i].rt < qs[cur] ? p[i].rt : qs[cur]);
        time += exec;
        p[i].rt -= exec;
        ...
        if (p[i].rt == 0) {
            ...
            int next_q = (cur < m-1) ? cur+1 : cur;
            q_push(&queues[next_q], i);
            p[i].q = next_q;
            in_queue[i] = 1;
        }
    }
    ...
    for (int i = 0; i < n; i++)
        printf("%d\t%d\t%d\t%d\t%d\t%d\t%d\n", 
               p[i].pid, p[i].at, p[i].bt, p[i].ct, p[i].tat, p[i].wt, p[i].rt_time);
}
...
int main() {
    ...
    for (int i = 0; i < m-1; i++) {
        printf("Q%d quantum: ", i);
        scanf("%d", &q[i]);
    }
    q[m-1] = 0; // Last queue FCFS
